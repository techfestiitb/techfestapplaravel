...
<head>
    ...
    <title>Bug Bounty Contest| Techfest, IIT Bombay</title>
    ...
    <meta property="og:image" content="http://techfest.org/2019/compi/bugbounty.jpg"/>
    ...
    <style>
        ...
        @media (min-width:992px) {
            ...
            .mobile_hide{
                ...
                background-image: url(/2019/compi/bugbounty.jpg) ;
...
<div id="colorlib-page"  >
    ...
    <aside id="colorlib-aside" role="complementary" class="border js-fullheight" >
        ...
        <a href="https://www.jlt.com/"><img  src="/2019/compi/images/jlt_logo.png" alt="" style="width: 100%; padding-top: 10%;"></a>
        <p style="color: white"><br>Disclaimer: <br>‘The JLT Group is now a part of the Marsh & McLennan Companies.’</p>
                ...
                <div class="row ">
                    ...
                    <div class="col-md-8 animate-box" data-animate-effect="fadeInLeft" style="margin-right:-1%; padding-left:2.5% ;padding-right: -50%">
                        <div class="about-desc" style="margin-right: 0%; padding: 0px" >
                            ...
                            <h2 style="color: white; font-weight: bold">Bug Bounty Contest</h2>
                            <hr style="color: white; border: 1px solid white">
                            <h3 style="color: white">Prize Money: INR 2,00,000/- </h3>

                            <p style=" font-size: 15px; width:90%; font-family: Lato;color: white ">
                                Ethical hacking bug bounty competition would require the participants to discover security vulnerabilities in an OS/server. This competition would test the capability of participants vulnerability seeking and understanding of softwares. In the current world, there is a need for ethical hacking awareness to safeguard the networks of businesses and softwares.</p>

                            @if(!empty($user_row->email))
                                <?php

                                $team =  DB::table('tf_reg')->where(['bugbounty_team' => $user_row->bugbounty_team])->get();
                                $team_leader_row = DB::table('tf_reg')->where(['email' => $team[0]->bugbounty_team])->first();

                                if($user_row->bugbounty_team == $user_row->email ){
                                    $team_leader = $user_row->name;
                                }
                                ?>
                            @endif

                            <a href="/2019/compi/ps/BUG-BOUNTY-CONTEST-updated.pdf" target="_blank" class="btn btn-primary btn-learn">Problem Statement</a>

                            @if(!empty($user_row->email))
                                @if($user_row->bugbounty == 0 or empty($user_row->bugbounty))
                                    {{--                                        if he is not registered--}}
{{--                                    <a href="/competitions/bugbounty/reg" class="btn btn-primary btn-learn" >Register</a>--}}
                                    <a href="#" class="btn btn-primary btn-learn" >Registration are closed</a>
                                @endif
                                @if($user_row->bugbounty == 1 && empty($user_row->bugbounty_team))
                                    <div class="dropdown ">
                                        <button class=" btn btn-primary btn-learn"><b>Team</b></button>
                                        <div class="dropdown-content">
                                            <a href="/competitions/bugbounty/createteam">Create Team</a>
                                            <a href="/competitions/bugbounty/jointeam">Join Team</a>
                                        </div>
                                    </div>
                                @endif

                                @if(!empty($user_row->bugbounty_team))
                                    @if($user_row->bugbounty_team != $user_row->email )
                                        <a href="/competitions/bugbounty/leaveteam" class="btn btn-primary btn-learn" >Leave current Team</a>
                                    @endif
                                    @if($user_row->bugbounty_team == $user_row->email )
                                        <a href="/competitions/bugbounty/dissolveteam" class="btn btn-primary btn-learn" >Dissolve the team</a>
                                        <div class="dropdown ">
                                            <button class=" btn btn-primary btn-learn">Remove a Member</button>
                                            <div class="dropdown-content">
                                                @if(!empty($team[0]->name) and ($team[0]->email != $team_leader_row->email))
                                                    <a href="/competitions/bugbounty/remove/{{$team[0]->id}}">{{$team[0]->name}}</a>
                                                @endif
                                                @if(!empty($team[1]->name)  and ($team[1]->email != $team_leader_row->email))
                                                    <a href="/competitions/bugbounty/remove/{{$team[1]->id}}">{{$team[1]->name}}</a>
                                                @endif
                                                @if(!empty($team[2]->name)  and ($team[2]->email != $team_leader_row->email))
                                                    <a href="/competitions/bugbounty/remove/{{$team[2]->id}}">{{$team[2]->name}}</a>
                                                @endif
                                                @if(!empty($team[3]->name) and ($team[3]->email != $team_leader_row->email))
                                                    <a href="/competitions/bugbounty/remove/{{$team[2]->id}}">{{$team[3]->name}}</a>
                                                @endif
                                            </div>
                                        </div>
                                    @endif
...
{{--                                <button class="btn btn-primary btn-learn" onclick="myFunction()">Register</button>--}}
                                <button class="btn btn-primary btn-learn">Registrations are closed</button>
                            ...
                            @if(!empty($user_row))
                                @if($user_row->bugbounty > 0 && empty($user_row->bugbounty_team))
                                    <div class="alert alert-success">
                                        <p><strong>Hi {{$user_row->name}}<br> Now create a team for BugBounty with <b>{{$user_row->email}}</b> as your Registered email id , now you must create/join a team.</strong></p>
                                    </div>
                                @endif
                                @if($user_row->bugbounty > 0 && !empty($user_row->bugbounty_team))
                                    <div class="" style="    border: solid 2px white;border-radius: 6px;padding: 2%;width: 95%">
                                        <p style="color: white">
                                            ...
                                            Team ID :BB- {{$team_leader_row->bugbounty_team_id}}
        ...
        <div id="text_change" class="col-md-12 animate-box scroll_inside" data-animate-effect="fadeInLeft" style="
            ...
            <p id="text_change" style="margin-bottom: 0px; color: white">
                <br> Stage 1 - <br>Submission of Abstract - Detailed functional description of the solution including proposed technical approach to develop it. Successful submissions must include detailed technical Specifications. <br><br> Stage 2 -
                <br>The proposed solution needs to be developed (prototyped) for alpha testing. The selected teams will have an opportunity to submit proposed approach and present their solution to JLT team to receive feedback. Successful submissions must have clearly outlined approach to develop the prototype along with an initial version of their prototype. <br> <br> Stage 3 -
                <br>Top 15 teams will be selected and would get the chance to participate in the Grand Finale at Techfest, IIT Bombay which is from 3rd-5th January. Here, the proposed prototype will be developed into working application for beta testing and demonstrated to a panel of judges. <br>
                <br></p>
        </div>
        {{--        </div>--}}
    </div>
...
<script>
    function myFunction0() {
        document.getElementById("text_change").innerHTML = "<br> Stage 1 - Submission of Abstract - Detailed functional description of the solution including proposed technical approach to develop it. Successful submissions must include detailed technical Specifications. <br><br> Stage 2 - The proposed solution needs to be developed (prototyped) for alpha testing. The selected teams will have an opportunity to submit proposed approach and present their solution to JLT team to receive feedback. Successful submissions must have clearly outlined approach to develop the prototype along with an initial version of their prototype. <br> <br> "3. Stage 3 - Top 15 teams will be selected and would get the chance to participate in the Grand Finale at Techfest, IIT Bombay which is from 3rd-5th January. Here, the proposed prototype will be developed into working application for beta testing and demonstrated to a panel of judges. <br><br>";
...
<script>
    function myFunction1() {
        document.getElementById("text_change").innerHTML =  "<br> Stage 1 - <br>Submission of Abstract -\n" +
                    "Detailed functional description of the solution including proposed technical approach to develop it. Successful submissions must include detailed technical Specifications.\n" +
                    "<br> <br>Stage 2 - <br>The proposed solution needs to be developed (prototyped) for alpha testing. The selected teams will have an opportunity to submit proposed approach and present their solution to JLT team to receive feedback. Successful submissions must have clearly outlined approach to develop the prototype along with an initial version of their prototype.\n" +
                    "<br> <br>Stage 3 - <br>Top 15 teams will be selected and would get the chance to participate in the Grand Finale at Techfest, IIT Bombay which is from 3rd-5th January. Here, the proposed prototype will be developed into working application for beta testing and demonstrated to a panel of judges.\n" +
...
<script>
    function myFunction2() {
        document.getElementById("text_change").innerHTML = "<br>Q. Why should I participate in Bug Bounty Contest?<br>" +
            "The competition provides its participants with a grasp of principles of ethical hacking and software vulnerabilities. <br>" +
            "<br>Q. How to register?\n" +
            "Follow this steps for registration : www.techfest.org > Competitions > Bug Bounty Contest > Explore More -> Register > Fill all your details > You will be registered and now you must either Create a team/Join a team.<br>" +
            ...
            "<br>And will be used in Adding Team members in a Team, Joining a Pre-existing team, availing accommodation, etc. " +
            "<br><br>Q. Can we add ourselves in any existing team?<br>" +
            "Yes, follow these steps to get added in a team. Join a team -> Enter required details -> You will be added in that team.\n" +
            "<br> <br>Q. How can I make my own team (as team leader)? <br>" +
            "You can choose any of the two ways to form your own team: Follow this process:- www.techfest.org -> Competitions -> Bug Bounty Contest -> Explore More -> Register -> Fill all your details -> Team -> Create a team -> Use the Registered Mail Id of other participants to add them in your team. They must be already registered before adding them in your Team.\n" +
            ...
            "<br> <br>Q. Is it nessecary for the team leader to add all the team members at once? <br>" +
            ...
            "<br>Q. Can we change the team leader of our team?\n" +
            "<br>No, you can’t change team leader of your team. But, you can dissolve the team and form a new one.<br>" +
...
<script>
    function myFunction3() {
         ...
         document.getElementById("text_change").innerHTML = " <br>" +
            "30th October -  Last date for registration<br>" +
            "30th October -  Last date for abstract submission  <br>" +
            "5th November - Results and Shortlisting of teams for Stage 2 <br>" +
            "12th November - Last date for submission of Technical report of Stage 2 <br>" +
            "25th November - Results and Shortlisting of teams for Grand Finale<br>" +
...
<script>
    function myFunction4() {
        document.getElementById("text_change").innerHTML = "<br>Yatin Mohan <br>+91 9417552777   <br> yatin.techfest@gmail.com<br><br> ";
    }
</script>
<style>
...
{{--                            <ul>--}}
{{--                                <li>Q. Why should I participate in Meshmerize?</li>--}}
