...
<head>
    ...
    <title>Earth Matters | Techfest, IIT Bombay</title>
    ...
    <style>
        ...
        @media (min-width:992px) {
            ...
            .mobile_hide{
                ...
                background-image: url(/2019/compi/images/earth.jpg) ;
...
<div id="colorlib-page"  >
    ...
    <aside id="colorlib-aside" role="complementary" class="border js-fullheight" >
        ...
        <a href="https://www.jlt.com/"><img  src="/2019/compi/images/LandT.jpg" alt="" style="width: 100%; padding-top: 10%;"></a>
                ...
                <div class="row ">
                    ...
                    <div class="col-md-8 animate-box" data-animate-effect="fadeInLeft" style="margin-right:-1%; padding-left:2.5% ;padding-right: -50%">
                        <div class="about-desc" style="margin-right: 0%; padding: 0px" >
                            ...
                            <h2 style="color: white; font-weight: bold">Earth Matters</h2>
                            <hr style="color: white; border: 1px solid white">
                            <h3 style="color: white">Prize Money: INR 1,25,000/- </h3>
...
{{--                                    {{$user_row->name}}, @endif--}}
                            ...
                            @if(!empty($user_row->email))
                                ...
                                $team =  DB::table('tf_reg')->where(['earthmatters_team' => $user_row->earthmatters_team])->get();
                                $team_leader_row = DB::table('tf_reg')->where(['email' => $team[0]->earthmatters_team])->first();

                                if($user_row->earthmatters_team == $user_row->email ){
                                    $team_leader = $user_row->name;
                                }
                                ?>
                            @endif

                            <a href="/2019/compi/ps/earthmatters.pdf" target="_blank" class="btn btn-primary btn-learn">Problem Statement</a>

                            @if(!empty($user_row->email))
                                @if($user_row->earthmatters == 0 or empty($user_row->earthmatters))
                                    {{--                                        if he is not registered--}}
{{--                                    <a href="/competitions/earthmatters/reg" class="btn btn-primary btn-learn" >Register</a>--}}
                                    <a href="#" class="btn btn-primary btn-learn" >Registrations Closed</a>
                                @endif
                                @if($user_row->earthmatters == 1 && empty($user_row->earthmatters_team))
                                    <div class="dropdown ">
                                        <button class=" btn btn-primary btn-learn"><b>Team</b></button>
                                        <div class="dropdown-content">
                                            <a href="/competitions/earthmatters/createteam">Create Team</a>
                                            <a href="/competitions/earthmatters/jointeam">Join Team</a>
                                        </div>
                                    </div>
                                @endif

                                @if(!empty($user_row->earthmatters_team))
                                    @if($user_row->earthmatters_team != $user_row->email )
                                        <a href="/competitions/earthmatters/leaveteam" class="btn btn-primary btn-learn" >Leave current Team</a>
                                    @endif
                                    @if($user_row->earthmatters_team == $user_row->email )
                                        <a href="/competitions/earthmatters/dissolveteam" class="btn btn-primary btn-learn" >Dissolve the team</a>
                                        <div class="dropdown ">
                                            <button class=" btn btn-primary btn-learn">Remove a Member</button>
                                            <div class="dropdown-content">
                                                @if(!empty($team[0]->name) and ($team[0]->email != $team_leader_row->email) )
                                                    <a href="/competitions/earthmatters/remove/{{$team[0]->id}}">{{$team[0]->name}}</a>
                                                @endif
                                                @if(!empty($team[1]->name) and ($team[1]->email != $team_leader_row->email) )
                                                    <a href="/competitions/earthmatters/remove/{{$team[1]->id}}">{{$team[1]->name}}</a>
                                                @endif
                                                @if(!empty($team[2]->name) and ($team[2]->email != $team_leader_row->email) )
                                                    <a href="/competitions/earthmatters/remove/{{$team[2]->id}}">{{$team[2]->name}}</a>
                                                @endif
                                                @if(!empty($team[3]->name) and ($team[3]->email != $team_leader_row->email) )
                                                    <a href="/competitions/earthmatters/remove/{{$team[2]->id}}">{{$team[3]->name}}</a>
                                                @endif
                                            </div>
                                        </div>
                                    @endif
                            ...
                            @if(!empty($user_row))
                                @if($user_row->earthmatters > 0 && empty($user_row->earthmatters_team))
                                    <div class="alert alert-success">
                                        <p><strong>Hi {{$user_row->name}}, you are successfully registered for Earth Matters with <b>{{$user_row->email}}</b> as your Registered email id , now you must create/join a team.</strong></p>
                                    </div>
                                @endif
                                @if($user_row->earthmatters > 0 && !empty($user_row->earthmatters_team))
                                    <div class="" style="    border: solid 2px white;border-radius: 6px;padding: 2%;width: 95%">
                                        <p style="color: white">
                                            ...
                                            Team ID :EM- {{$team_leader_row->earthmatters_team_id}}
        ...
        <div id="text_change" class="col-md-12 animate-box scroll_inside" data-animate-effect="fadeInLeft" style="
            ...
            <p id="text_change" style="margin-bottom: 0px; color: white">
                ...
                <br><br>Water for All
                <br>▪ Development and maintenance of reservoirs andnatural & artificial catchment
                systems
                <br>▪ Reduction, filtration and recycling of sewage water
                <br>▪ Profiling of water rich and water scarce areas
                <br>▪ Transportation of water from water rich to water scarce areas
                <br>▪ Energy and cost efficient desalination technique
                <br><br> OR
                <br><br>Mission Zero Waste
                <br>▪ Eco-friendly technology for recycling e-waste that can also recover the resources
                gone into it
                <br>▪ Reuse of electronic materials
                <br>▪ Reduction in e-waste & plastic consumption and smart waste disposal system
                <br>▪ Making water bodies plastic free
                <br>▪ Alternatives to plastic
                <br><br> OR
                <br><br>Clean Environment
                <br>▪ Reducing emission of Greenhouse gases
                <br>▪ Environmental sanitation
                <br>▪ Chemicals and effluents flowing into water bodies
                <br>▪ Renewable sources of energy with low infrastructural requirements
                <br>▪ Organic agriculture
                <br>▪ Cost effective air purifiers
                <br><br></p>
        </div>
        {{--        </div>--}}
    </div>
...
<script>
    function myFunction1() {
        document.getElementById("text_change").innerHTML = "<br><u>ABSTRACT FORMAT</u>\n <br>" +
            ...
            "\n" +
            "<br><br>The project report should be emailed to earthmatters@techfest.org with the subject ‘Ideate: ‘’Earth Matters’’ Project Report: Team Id’ (for eg. Ideate: ‘’Earth Matters’’ Project Report: EM1234).\n" +
            "All the submissions should be emailed to earthmatters@techfest.org. Teams must follow the following details for the submission:\n" +
...
<script>
    function myFunction2() {
        document.getElementById("text_change").innerHTML = "<br>Q. Who can participate ?\n" +
            ...
            "www.techfest.org > Ideates > Earth Matters > Explore More -> Register > Fill all your details > You will be registered and now you must either Create a team/Join a team<br>" +
            ...
            "Follow this process:- www.techfest.org -> Competitions -> Earth Matters -> Explore More -> Register -> Fill all your details -> Team -> Create a team -> Use the Registered Mail Id of other participants to add them in your team. They must be already registered before adding them in your Team. \n<br>" +
...
<script>
    function myFunction3() {
         ...
         document.getElementById("text_change").innerHTML = "<br> " +
             // "<br> IMPORTANT NOTE:" +
        //     "<br><br><u>Intellectual Property Rights</u>\n" +
        //     "<br>The IP rights in the content(s) of the submitted entries and related prototype shall be assigned to L&T Electrical & Automation (“L&T E&A”) without any further consideration. L&T E&A shall have the rights to implement and commercialize the innovative solution submitted in the entries and prototype on industrial and manufacturing level.\n" +
        //     "<br>Participants must send their signed Declaration Form to earthmatters@techfest.org to be considered eligible for participating in the ideate-Earth Matters. Otherwise they will be disqualified.\n" +
        //     "<br>Click here to download the Declaration Form" +
            "<u>GENERAL RULES</u>\n" +
            "\n" +
            ...
            "\n" +
            "<br><br><u>EVALUATION</u>\n" +
            "\n" +
            "<br><br>Earth Matters abstracts will be judged by a panel of experts. Following are the broad guidelines for judging:\n" +
            "<br>1.Creativity and Novelty: How novel is the idea? How different is it from the current solutions available? The innovation must be ingenious and novel in its area of application and should have a high potential for leaving an impact on the society.\n" +
            " <br>2.Originality: The innovation should not, by any means,  include copied or stolen work, such applications will be disqualified immediately.\n" +
            " <br>3.Performance\n" +
            " <br>4.Cost/Market Value and Acceptance\n" +
            " <br>5.Durability and Usability: Durability of the prototype/method proposed.\n" +
            " <br>6.Implementation ability: Is the solution implementable as described? Is it repeatable? Is the solution feasible for diverse and changing conditions?\n" +
            " <br>7.Scalability: Is the solution scalable to a higher level, how easy is it to scale up and what are the factors affecting it?\n" +
            " <br>8.Potential of Impact: How does it benefit society? The Scale of problem it solves, Intensity of the solution and number of people catered from the solution directly and indirectly.\n" +
            " <br>9.Design: Has the design been taken into consideration? How optimized is the product?\n" +
            " <br>10.Ergonomics If the team decides to make a well-designed product\n" +
...
<script>
    function myFunction4() {
        document.getElementById("text_change").innerHTML = "<br>Atharv Toraskar <br>+91 8369077493 <br> atharv.techfest19@gmail.com<br><br> ";
    }
</script>
<style>
