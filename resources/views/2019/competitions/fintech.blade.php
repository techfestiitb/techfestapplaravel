...
<head>
    ...
    <title>OPPO FinTech Contest | Techfest, IIT Bombay</title>

    <link rel="shortcut icon" type="image/x-icon" href="/2019/ca/images/favicon_logo.png" />
    <meta property="og:image" content="http://techfest.org/2019/compi/oppo_fintech.png" />
    ...
    <style>
        ...
        @media (min-width:992px) {
            ...
            .mobile_hide{
                ...
                background-image: url(/2019/compi/oppo_fintech.png) ;
...
<div id="colorlib-page"  >
    ...
    <aside id="colorlib-aside" role="complementary" class="border js-fullheight">
        ...
        <a href="#"><img  src="/2019/compi/images/oppo_fintech.png" alt="" style="width: 100%; padding-top: 10%;"></a>
                    ...
                    <div class="col-md-8 animate-box" data-animate-effect="fadeInLeft" style="margin-right:-1%; padding-left:2.5% ;padding-right: -50%">
                        <div class="about-desc" style="margin-right: 0%; padding: 0px" >
                            ...
                            <div style="display: inline">
                                <h2 style="color: white; font-weight: bold;display: inline">OPPO FinTech Contest</h2>
                                {{--                                <a href="https://www.jlt.com/"><img  src="/2019/compi/images/jlt_logo.png" alt="" style="max-height: 6vh;margin-top: -13px;"></a>--}}
                            </div>

                            <hr style="color: white; border: 1px solid white">
{{--                            <h3 style="color: white">Prize Money: INR 1,75,000/- </h3>--}}

                            <p style=" font-size: 15px; width:90%; font-family: Lato;color: white; text-align: justify ">
                                Innovate to make the processes in fintech world simpler and more efficient. <br>
                                Develop application(s) to <br>
                                (1) Fill application form without typing <br>
                                (2) Give product recommendation using transaction pattern <br>
                                (3) Recommend meeting zone and agent for a customer
                            </p>

                            @if(!empty($user_row->email))
                                <?php

                                $team =  DB::table('tf_reg')->where(['fintech_team' => $user_row->fintech_team])->get();
                                $team_leader_row = DB::table('tf_reg')->where(['email' => $team[0]->fintech_team])->first();
                                if($user_row->fintech_team == $user_row->email ){
                                    $team_leader = $user_row->name;
                                }
                                ?>
                            @endif

                            <a href="/2019/compi/ps/OPPO-FinTech-Contest.pdf" target="_blank" class="btn btn-primary btn-learn">Problem Statement</a>
                                ...
                                @if($user_row->fintech == 1 and empty($user_row->fintech_team))
                                    <div class="dropdown ">
                                        ...
                                        <div class="dropdown-content">
                                            <a href="/competitions/fintech/createteam">Create Team</a>
                                            <a href="/competitions/fintech/jointeam">Join Team</a>
                                        </div>
                                    </div>
                                @endif

                                @if(!empty($user_row->fintech_team))
                                    @if($user_row->fintech_team != $user_row->email )
                                        <a href="/competitions/fintech/leaveteam" class="btn btn-primary btn-learn" >Leave current Team</a>
                                    @endif
                                    @if($user_row->fintech_team == $user_row->email )
                                        <a href="/competitions/fintech/dissolveteam" class="btn btn-primary btn-learn" >Dissolve the team</a>
                                        <div class="dropdown ">
                                            ...
                                            <div class="dropdown-content">
                                                @if(!empty($team[0]->email) and ($team[0]->email != $team_leader_row->email))
                                                    <a href="/competitions/fintech/remove/{{$team[0]->id}}">{{$team[0]->name}}</a>
                                                @endif
                                                @if(!empty($team[1]->name) and ($team[1]->email != $team_leader_row->email))
                                                    <a href="/competitions/fintech/remove/{{$team[1]->id}}">{{$team[1]->name}}</a>
                                                @endif
                                                @if(!empty($team[2]->name) and ($team[2]->email != $team_leader_row->email))
                                                    <a href="/competitions/fintech/remove/{{$team[2]->id}}">{{$team[2]->name}}</a>
                                                @endif
                                                @if(!empty($team[3]->name) and ($team[3]->email != $team_leader_row->email))
                                                    <a href="/competitions/fintech/remove/{{$team[2]->id}}">{{$team[3]->name}}</a>
                                                @endif
                                            </div>
                                        </div>
                                    @endif
                            ...
                            @if(!empty($user_row))
                                @if($user_row->fintech > 0 and empty($user_row->fintech_team))
                                    <div class="alert alert-warning">
                                        <p><strong>Hi {{$user_row->name}}<br> Now click on Team and then click on either Create Team (to create  your team) OR Join Team (to join an existing team).
                                                <br> <b>{{$user_row->email}}</b> is your Registered email id</strong></p>
                                    </div>
                                @endif
                                @if($user_row->fintech > 0 and !empty($user_row->fintech_team))
                                    <div class="" style="    border: solid 2px white;border-radius: 6px;padding: 2%;width: 95%">
                                        <p style="color: white">
                                            ...
                                            Team ID :FT - {{$team_leader_row->fintech_team_id}}
        ...
        <div id="text_change" class="col-md-12 animate-box scroll_inside" data-animate-effect="fadeInLeft" style="
            ...
            <p id="text_change" style="margin-bottom: 0px; color: white">
                <br>Stage 1 - Submission of a detailed functional description of the solution including the proposed technical approach to develop it. Successful submissions must include detailed technical specifications.
                <br>
                <br>Stage 2 - Selected teams will move to Stage 2. Here the proposed solution needs to be developed.<br>
                <br>Stage 3 - Selected teams would be invited to Techfest, IIT Bombay which is from 3rd-5th January 2020. Here, the proposed prototype will be demonstrated to a panel of judges
...
<script>
    function myFunction1() {
        document.getElementById("text_change").innerHTML = "<br>Stage 1 - Submission of a detailed functional description of the solution including the proposed technical approach to develop it. Successful submissions must include detailed technical specifications.<br>" +
        "<br>Stage 2 - Selected teams will move to Stage 2. Here the proposed solution needs to be  developed.\n" +
            "<br> <br>Stage 3 - Selected teams would be invited to Techfest, IIT Bombay which is from 3rd-5th January 2020. Here, the proposed prototype will be demonstrated to a panel of judges  <br> <br>";
...
<script>
    function myFunction2() {
        ...
        document.getElementById("text_change").innerHTML = "<br>Q. Why should I participate in OPPO FinTech Contest?" +
            "<br>The competition provides its participants with a grasp of principles of fintech and app development. This will help them strengthen their skills in software  integration with finance." +
            "<brQ. >How to register" +
            "<br>Follow these steps for registration : www.techfest.org > Competitions > OPPO FinTech Contest> Explore More -> Register > Fill all your details > You will be registered and now you must either Create a team/Join a team" +
            "<br><br>Q. How many people can be there in one team?" +
            "<br>One team can have maximum of 4 members." +
            "<br><br>Q. What is the Registered Mail Id of a person?" +
            "<br>The Gmail Id which was used to Sign In is the Registered mail Id. It is an Unique Identity of any participant. And will be used in Adding Team members in a Team, Joining a Pre-existing team, availing accommodation, etc." +
            "<br><br>Q. Can anyone add members in a team?" +
            "<br>No, only the team leader has the right to add members in a team. But others can join an existing team by clicking on Join a Team button." +
            "<br><br>Q. Can we add ourselves in any existing team?" +
            "<br>Yes, follow these steps to get added in a team. Join a team -> Enter required details -> You will be added in that team" +
            "<br><br>Q. How can I make my own team (as team leader)?" +
            "<br>You can choose any of the two ways to form your own team: You can choose any of the two ways to form your own team: Follow this process:- www.techfest.org -> Competitions -> OPPO FinTech Contest -> Explore More -> Register -> Fill all your details -> Team -> Create a team -> Use the Registered Mail Id of other participants to add them in your team. They must be already registered before adding them in your Team." +
            "<br><br>Q. How can a team leader remove members from the team?" +
            "<br>Click on Remove Member button and then remove the member you want to remove." +
            "<br><br>Q. Is it necessary for the team leader to add all the team members at once?" +
            "<br>No, it is not necessary. Team leader can add 2 members or less than 2, only once which is while creating the team but if the team is not full and he/she wants to add more members afterwards, then members (to be added) will have to use the Join Team button to join team." +
            "<br><br>Q. How can a team member(other than team leader) leave his/her team?" +
            "<br>Click on Leave Team button and you will be removed from the registered team." +
            "<br><br>Q. Who can delete a team?" +
            "<br>Only team leader can delete the team using the Dissolve Team button." +
            "<br><br>Q. Can we change the team leader of our team?" +
            "<br>No, you can’t change team leader of your team. But, you can dissolve the team and form a new one." +
            "<br><br>Q. Can a team have members from different colleges?" +
            "<br>Yes, students from different colleges can form a team." +
            "<br><br>Q. How many stages does the competition have?" +
            "<br>The competition has 3 stages. Top  teams will qualify for the Grand Finale at Techfest 2019-20." +
...
<script>
    function myFunction3() {
        ...
        document.getElementById("text_change").innerHTML = " <br>" +
            "23rd October -  Last date for registration<br>" +
            "24th October -  Last date for theoretical report of Stage 1 <br>" +
            "25th October -  Results and Shortlisting of teams for Stage 2  <br>" +
            "25th November - Last date for submission of design report of Stage 2 <br>" +
            "5th December - Results and Shortlisting of teams for Grand Finale <br>" +
            "Improvisation stage: 6th December,2019 - 2nd January,2020<br>" +
            "3rd-5th January - Grand Finale and presentation of complete prototype <br><br>";
        document.body.click("body");
...
<script>
    function myFunction4() {
        document.getElementById("text_change").innerHTML = "<br>Roshan Jangid | +91 9130671636 | roshanj.techfest@gmail.com <br> Yash Choudhary | +91 7976302312 | yashchoudhary.techfest@gmail.com <br><br> ";
        document.body.click("body");
