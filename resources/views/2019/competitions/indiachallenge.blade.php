...
<head>
    ...
    <title>JLT India Challenge | Techfest, IIT Bombay</title>

    <link rel="shortcut icon" type="image/x-icon" href="/2019/ca/images/favicon_logo.png" />
    <meta property="og:image" content="http://techfest.org/2019/compi/images/indiachallenge.png" />
    ...
    <style>
        ...
        @media (min-width:992px) {
            ...
            .mobile_hide{
                ...
                background-image: url(/2019/compi/images/indiachallenge.png) ;
                    ...
                    <div class="col-md-8 animate-box" data-animate-effect="fadeInLeft" style="margin-right:-1%; padding-left:2.5% ;padding-right: -50%">
                        <div class="about-desc" style="margin-right: 0%; padding: 0px" >
                            ...
                            <div style="display: inline">
                                <h2 style="color: white; font-weight: bold;display: inline">JLT India Challenge</h2>
{{--                                <a href="https://www.jlt.com/"><img  src="/2019/compi/images/jlt_logo.png" alt="" style="max-height: 6vh;margin-top: -13px;"></a>--}}
                            ...
                            <p style=" font-size: 15px; width:90%; font-family: Lato;color: white; text-align: justify ">
                                <u>DocAssist</u> <br> Develop a software application which assists in document creation using pre-defined templates involving Data analytics and ML. As part of business transactions, one has to create multiple key documents to cater to customers, stakeholders, employees, etc. There is a need to automate creation of these standard key documents.
                                </p>

                            @if(!empty($user_row->email))
                                <?php

                                $team =  DB::table('tf_reg')->where(['indiachallenge_team' => $user_row->indiachallenge_team])->get();
                                $team_leader_row = DB::table('tf_reg')->where(['email' => $team[0]->indiachallenge_team])->first();
                                if($user_row->indiachallenge_team == $user_row->email ){
                                    $team_leader = $user_row->name;
                                }
                                ?>
                            @endif

                            <a href="/2019/compi/ps/JLT-India-Challenge.pdf" target="_blank" class="btn btn-primary btn-learn">Problem Statement</a>

                            @if(!empty($user_row->email))
                                @if($user_row->indiachallenge == 0 or empty($user_row->indiachallenge))
                                    {{--                                        if he is not registered--}}
                                    <a href="#" class="btn btn-primary btn-learn" >Registeration Closed</a>
                                @endif
                                @if($user_row->indiachallenge == 1 and empty($user_row->indiachallenge_team))
                                    <div class="dropdown ">
                                        <button class=" btn btn-primary btn-learn"><b>Team</b></button>
                                        <div class="dropdown-content">
                                            <a href="/competitions/indiachallenge/createteam">Create Team</a>
                                            <a href="/competitions/indiachallenge/jointeam">Join Team</a>
                                        </div>
                                    </div>
                                @endif

                                @if(!empty($user_row->indiachallenge_team))
                                    @if($user_row->indiachallenge_team != $user_row->email )
                                        <a href="/competitions/indiachallenge/leaveteam" class="btn btn-primary btn-learn" >Leave current Team</a>
                                    @endif
                                    @if($user_row->indiachallenge_team == $user_row->email )
                                        <a href="/competitions/indiachallenge/dissolveteam" class="btn btn-primary btn-learn" >Dissolve the team</a>
                                        <div class="dropdown ">
                                            ...
                                            <div class="dropdown-content">
                                                @if(!empty($team[0]->email) and ($team[0]->email != $team_leader_row->email))
                                                    <a href="/competitions/indiachallenge/remove/{{$team[0]->id}}">{{$team[0]->name}}</a>
                                                @endif
                                                @if(!empty($team[1]->name) and ($team[1]->email != $team_leader_row->email))
                                                    <a href="/competitions/indiachallenge/remove/{{$team[1]->id}}">{{$team[1]->name}}</a>
                                                @endif
                                                @if(!empty($team[2]->name) and ($team[2]->email != $team_leader_row->email))
                                                    <a href="/competitions/indiachallenge/remove/{{$team[2]->id}}">{{$team[2]->name}}</a>
                                                @endif
                                                @if(!empty($team[3]->name) and ($team[3]->email != $team_leader_row->email))
                                                    <a href="/competitions/indiachallenge/remove/{{$team[2]->id}}">{{$team[3]->name}}</a>
                                                @endif
                                            </div>
                                        </div>
                                    @endif
                            ...
                            @if(!empty($user_row))
                                @if($user_row->indiachallenge > 0 and empty($user_row->indiachallenge_team))
                                    <div class="alert alert-success">
                                        <p><strong>Hi {{$user_row->name}}<br> Now create a team for India Challenge with <b>{{$user_row->email}}</b> as your Registered email id , now you must create/join a team.</strong></p>
                                    </div>
                                @endif
                                @if($user_row->indiachallenge > 0 and !empty($user_row->indiachallenge_team))
                                    <div class="" style="    border: solid 2px white;border-radius: 6px;padding: 2%;width: 95%">
                                        <p style="color: white">
                                            ...
                                            Team ID :IC - {{$team_leader_row->indiachallenge_team_id}}
        ...
        <div id="text_change" class="col-md-12 animate-box scroll_inside" data-animate-effect="fadeInLeft" style="
            ...
            <p id="text_change" style="margin-bottom: 0px; color: white">
                <br>Stage 1 - Submission of detailed functional description of the solution including proposed technical approach to develop it. Successful submissions must include detailed technical Specifications.<br>
                ...
                proposed prototype will be developed into working application for beta testing and demonstrated to a panel of judges.
                <br>
            </p>
        </div>
...
<script>
    function myFunction2() {
        ...
        document.getElementById("text_change").innerHTML = "<br>Q. Why should I participate in JLT India Challenge?<br>" +
            "The competition provides its participants with a grasp of principles of Data Analytics and embedded systems. This will help them strengthen their skills in hardware and software of embedded devices using the principles of Data Analytics. <br>" +
            "<br>Q. How to register?<br>Follow this steps for registration : www.techfest.org > Competitions > JLT India Challenge > Explore More -> Register > Fill all your details > You will be registered and now you must either Create a team/Join a team <br>" +
            ...
            "You can choose any of the two ways to form your own team: Follow this process:- www.techfest.org -> Competitions -> JLT India Challenge -> Explore More -> Register -> Fill all your details -> Team -> Create a team -> Use the Registered Mail Id of other participants to add them in your team. They must be already registered before adding them in your Team. \n<br>" +
            ...
            "<br>The competition has two stages: namely zonal qualifier and finale. Top five teams from Each Zonal Qualifier will qualify for the Grand Finale at Techfest 2019-20." +
...
<script>
    function myFunction3() {
        ...
        document.getElementById("text_change").innerHTML = " <br>" +
            "23th October -  Last date for registration<br>" +
            "23th October -  Last date for theoretical report of Stage 1 <br>" +
            "25th October -  Results and Shortlisting of teams for Stage 2  <br>" +
            "10rd November - Last date for submission of design report of Stage 2 <br>" +
            "30th November - Results and Shortlisting of teams for Grand Finale <br>" +
