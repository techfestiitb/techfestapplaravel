...
<head>
    ...
    <title>Innovationeering | Techfest</title>
    ...
    <style>
        ...
        @media (min-width:992px) {
            ...
            .mobile_hide{
                ...
                background-image: url(/2019/compi/images/bulb.jpg) ;
                ...
                <div class="row ">
                    ...
                    <div class="col-md-8 animate-box" data-animate-effect="fadeInLeft" style="margin-right:-1%; padding-left:2.5% ;padding-right: -50%">
                        <div class="about-desc" style="margin-right: 0%; padding: 0px" >
                            ...
                            <h2 style="color: white; font-weight: bold">Innovationeering</h2>
...
{{--                                    {{$user_row->name}}, @endif--}}
                            ...
                            @if(!empty($user_row->email))
                                ...
                                $team =  DB::table('tf_reg')->where(['innovationeering_team' => $user_row->innovationeering_team])->get();
                                $team_leader_row = DB::table('tf_reg')->where(['email' => $team[0]->innovationeering_team])->first();

                                if($user_row->innovationeering_team == $user_row->email ){
                                    $team_leader = $user_row->name;
                                }
                                ?>
                            @endif

                            <a href="/2019/compi/ps/innovationeering.pdf" target="_blank" class="btn btn-primary btn-learn">Problem Statement</a>

                            @if(!empty($user_row->email))
                                @if($user_row->innovationeering == 0 or empty($user_row->innovationeering))
                                    {{--                                        if he is not registered--}}
{{--                                    <a href="/competitions/innovationeering/reg" class="btn btn-primary btn-learn" >Register</a>--}}
                                    <a href="#" class="btn btn-primary btn-learn" >Registrations Closed</a>
                                @endif
                                @if($user_row->innovationeering == 1 && empty($user_row->innovationeering_team))
                                    <div class="dropdown ">
                                        <button class=" btn btn-primary btn-learn"><b>Team</b></button>
                                        <div class="dropdown-content">
                                            <a href="/competitions/innovationeering/createteam">Create Team</a>
                                            <a href="/competitions/innovationeering/jointeam">Join Team</a>
                                        </div>
                                    </div>
                                @endif

                                @if(!empty($user_row->innovationeering_team))
                                    @if($user_row->innovationeering_team != $user_row->email )
                                        <a href="/competitions/innovationeering/leaveteam" class="btn btn-primary btn-learn" >Leave current Team</a>
                                    @endif
                                    @if($user_row->innovationeering_team == $user_row->email )
                                        <a href="/competitions/innovationeering/dissolveteam" class="btn btn-primary btn-learn" >Dissolve the team</a>
                                        <div class="dropdown ">
                                            <button class=" btn btn-primary btn-learn">Remove a Member</button>
                                            <div class="dropdown-content">
                                                @if(!empty($team[0]->name) and ($team[0]->email != $team_leader_row->email) )
                                                    <a href="/competitions/innovationeering/remove/{{$team[0]->id}}">{{$team[0]->name}}</a>
                                                @endif
                                                @if(!empty($team[1]->name) and ($team[1]->email != $team_leader_row->email))
                                                    <a href="/competitions/innovationeering/remove/{{$team[1]->id}}">{{$team[1]->name}}</a>
                                                @endif
                                                @if(!empty($team[2]->name) and ($team[2]->email != $team_leader_row->email) )
                                                    <a href="/competitions/innovationeering/remove/{{$team[2]->id}}">{{$team[2]->name}}</a>
                                                @endif
                                                @if(!empty($team[3]->name) and ($team[3]->email != $team_leader_row->email) )
                                                    <a href="/competitions/innovationeering/remove/{{$team[2]->id}}">{{$team[3]->name}}</a>
                                                @endif
                                            </div>
                                        </div>
                                    @endif
...
{{--                                <a href="/2019/compi/ps/How to register.pdf" style="color: white">Want to know how to register - Click here</a>--}}
                            ...
                            @if(!empty($user_row))
                                @if($user_row->innovationeering > 0 && empty($user_row->innovationeering_team))
                                    <div class="alert alert-success">
                                        <p><strong>Hi {{$user_row->name}}<br> Now create a team for Innovationeering with <b>{{$user_row->email}}</b> as your Registered email id , now you must create/join a team.</strong></p>
                                    </div>
                                @endif
                                @if($user_row->innovationeering > 0 && !empty($user_row->innovationeering_team))
                                    <div class="" style="    border: solid 2px white;border-radius: 6px;padding: 2%;width: 95%">
                                        <p style="color: white">
                                            ...
                                            Team ID :IN- {{$team_leader_row->innovationeering_team_id}}
        ...
        <div id="text_change" class="col-md-12 animate-box scroll_inside" data-animate-effect="fadeInLeft" style="
            ...
            <p id="text_change" style="margin-bottom: 0px; color: white">
                ...
                <br><br>Healthcare
                <br>▪ Primary healthcare services
                <br>▪ Sanitation biotechnology
                <br>▪ Efficient and well trained workforce reaching the last mile
                <br>▪ Malnutrition in children
                <br>▪ Poor standards of hygiene
                <br>▪ Indigenous healthcare systems
                <br><br> OR
                <br><br>Education for Underprivileged Children
                <br>▪ Good infrastructure in schools
                <br>▪ Local language, quality study material and trained teachers
                <br>▪ Gender equality and disabled children
                <br>▪ Transportation to schools
                <br>▪ Nutrition and education expenses
                <br><br> OR
                <br><br>Electricity
                <br>▪ Generation of electricity from waste material
                <br>▪ Rural electrification
                <br>▪ Non polluting and renewable sources of energy
                <br>▪ Smart grid infrastructure with quality, reliable and secure power supply
                <br>▪ Self sustainable street lights
                <br>▪ Energy storage systems<br>
                <br></p>
        </div>
        {{--        </div>--}}
    </div>
...
<script>
    function myFunction1() {
        document.getElementById("text_change").innerHTML = "<br><u>ABSTRACT FORMAT</u>\n <br>" +
            ...
            "\n" +
            "<br><br>The project report should be emailed to manjeet@techfest.org with the subject ‘Ideate: ‘’Innovationeering’’ Project Report: Team Id’ (for eg. Ideate: ‘’Innovationeering’’ Project Report: IN1234).\n" +
            "All the submissions should be emailed to manjeet@techfest.org. Teams must follow the following details for the submission:\n" +
...
<script>
    function myFunction2() {
        document.getElementById("text_change").innerHTML = "<br>Q. Who can participate ?\n" +
            ...
            "www.techfest.org > Ideates > Innovationeering > Explore More -> Register > Fill all your details > You will be registered and now you must either Create a team/Join a team<br>" +
            ...
            "Follow this process:- www.techfest.org -> Competitions -> Innovationeering -> Explore More -> Register -> Fill all your details -> Team -> Create a team -> Use the Registered Mail Id of other participants to add them in your team. They must be already registered before adding them in your Team. \n<br>" +
...
<script>
    function myFunction3() {
        ...
        document.getElementById("text_change").innerHTML = "<br><u>GENERAL RULES </u> \n" +
            "\n" +
            ...
            "\n" +
            "<br><br>Innovationeering abstracts will be judged by a panel of experts. Following are the broad guidelines for judging:\n" +
            ...
            "<br><br>In case of any discrepancies, the decision of the Organizers or Judges will be final and binding on all.\n<br><br>";
    }
</script>

<script>
    function myFunction4() {
        document.getElementById("text_change").innerHTML = "<br> Shubham Anand <br>+91 6204447516 <br> shubhamanand.techfest@gmail.com<br> <br>";
    }
</script>
<style>
