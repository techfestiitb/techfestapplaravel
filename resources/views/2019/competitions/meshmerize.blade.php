                ...
                <div class="row ">
                    ...
                    <div class="col-md-8 animate-box" data-animate-effect="fadeInLeft" style="margin-right:-1%; padding-left:2.5% ;padding-right: -50%">
                        <div class="about-desc" style="margin-right: 0%; padding: 0px" >
                            ...
                            <h2 style="color: white; font-weight: bold">Meshmerize</h2>
                            ...
                            @if(!empty($user_row->email))
                                ...
                                $team =  DB::table('tf_reg')->where(['meshmerize_team' => $user_row->meshmerize_team])->get();
                                $team_leader_row = DB::table('tf_reg')->where(['email' => $team[0]->meshmerize_team])->first();

                                if($user_row->meshmerize_team == $user_row->email ){
                                    $team_leader = $user_row->name;
                                }
                                ?>
                            @endif

                            <a href="/2019/compi/ps/meshmerize.pdf" target="_blank" class="btn btn-primary btn-learn">Problem Statement</a>

                            @if(!empty($user_row->email))
                                @if($user_row->meshmerize == 0 or empty($user_row->meshmerize))
                                    {{--                                        if he is not registered--}}
                                    <a href="/competitions/meshmerize/reg" class="btn btn-primary btn-learn" >Register</a>
                                @endif
                                @if($user_row->meshmerize == 1 && empty($user_row->meshmerize_team))
                                    <div class="dropdown ">
                                        <button class=" btn btn-primary btn-learn"><b>Team</b></button>
                                        <div class="dropdown-content">
                                            <a href="/competitions/meshmerize/createteam">Create Team</a>
                                            <a href="/competitions/meshmerize/jointeam">Join Team</a>
                                        </div>
                                    </div>
                                @endif

                                @if(!empty($user_row->meshmerize_team))
                                    @if($user_row->meshmerize_team != $user_row->email )
                                        <a href="/competitions/meshmerize/leaveteam" class="btn btn-primary btn-learn" >Leave current Team</a>
                                    @endif
                                    @if($user_row->meshmerize_team == $user_row->email )
                                        <a href="/competitions/meshmerize/dissolveteam" class="btn btn-primary btn-learn" >Dissolve the team</a>
                                        <div class="dropdown ">
                                            <button class=" btn btn-primary btn-learn">Remove a Member</button>
                                            <div class="dropdown-content">
                                                @if(!empty($team[0]->name) and ($team[0]->email != $team_leader_row->email))
                                                    <a href="/competitions/meshmerize/remove/{{$team[0]->id}}">{{$team[0]->name}}</a>
                                                @endif
                                                @if(!empty($team[1]->name)  and ($team[1]->email != $team_leader_row->email))
                                                    <a href="/competitions/meshmerize/remove/{{$team[1]->id}}">{{$team[1]->name}}</a>
                                                @endif
                                                @if(!empty($team[2]->name)  and ($team[2]->email != $team_leader_row->email))
                                                    <a href="/competitions/meshmerize/remove/{{$team[2]->id}}">{{$team[2]->name}}</a>
                                                @endif
                                                @if(!empty($team[3]->name) and ($team[3]->email != $team_leader_row->email))
                                                    <a href="/competitions/meshmerize/remove/{{$team[2]->id}}">{{$team[3]->name}}</a>
                                                @endif
                                            </div>
                                        </div>
                                    @endif
                            ...
                            @endif
                            @if(empty($user_row->email))
                                <button class="btn btn-primary btn-learn " id="signinButton" >Register/sign in </button>
                            @endif
                            <div id="snackbar">Sign in to register</div>
                            @if(!empty($user_row))
                                @if($user_row->meshmerize > 0 && empty($user_row->meshmerize_team))
                                ...
                                @if($user_row->meshmerize > 0 && !empty($user_row->meshmerize_team))
                                    <div class="" style="    border: solid 2px white;border-radius: 6px;padding: 2%;width: 95%">
                                        <p style="color: white">
                                            ...
                                            Team ID :MH- {{$team_leader_row->meshmerize_team_id}}
