                ...
                <div class="row ">
                    ...
                    <div class="col-md-8 animate-box" data-animate-effect="fadeInLeft" style="margin-right:-1%; padding-left:2.5% ;padding-right: -50%">
                        <div class="about-desc" style="margin-right: 20%; padding: 0px" >
                            ...
                            <h2 style="color: white; font-weight: bold">CoDecode (Wildcard)</h2>
                            ...
                            @if(!empty($user_row->email))
                                ...
                                $team =  DB::table('tf_reg')->where(['wcodecode_team' => $user_row->wcodecode_team])->get();
                                $team_leader_row = DB::table('tf_reg')->where(['email' => $team[0]->wcodecode_team])->first();

                                if($user_row->wcodecode_team == $user_row->email ){
                            ...
                            @if(!empty($user_row->email))
                                @if($user_row->wcodecode == 0 or empty($user_row->wcodecode))
                                    {{--                                        if he is not registered--}}
                                    <a href="/competitions/wcodecode/reg" class="btn btn-primary btn-learn " >Register (Wildcard)</a>
                                @endif
                                @if($user_row->wcodecode == 1 && empty($user_row->wcodecode_team))
                                    <div class="dropdown ">
                                        ...
                                        <div class="dropdown-content">
                                            ...
                                            <a href="/competitions/wcodecode/createteam">Create Team</a>
                                            <a href="/competitions/wcodecode/jointeam">Join Team</a>
                                        </div>
                                    </div>
                                @endif

                                @if(!empty($user_row->wcodecode_team))
                                    @if($user_row->wcodecode_team != $user_row->email )
                                        <a href="/competitions/wcodecode/leaveteam" class="btn btn-primary btn-learn" >Leave current Team</a>
                                    @endif
                                    @if($user_row->wcodecode_team == $user_row->email )
                                        <a href="/competitions/wcodecode/dissolveteam" class="btn btn-primary btn-learn" >Dissolve the team</a>
                                        <div class="dropdown ">
                                            <button class=" btn btn-primary btn-learn">Remove a Member</button>
                                            <div class="dropdown-content">
                                                @if(!empty($team[0]->name) and ($team[0]->email != $team_leader_row->email) )
                                                    <a href="/competitions/wcodecode/remove/{{$team[0]->id}}">{{$team[0]->name}}</a>
                                                @endif
                                                @if(!empty($team[1]->name) and ($team[1]->email != $team_leader_row->email))
                                                    <a href="/competitions/wcodecode/remove/{{$team[1]->id}}">{{$team[1]->name}}</a>
                                                @endif
                                                @if(!empty($team[2]->name) and ($team[2]->email != $team_leader_row->email))
                                                    <a href="/competitions/wcodecode/remove/{{$team[2]->id}}">{{$team[2]->name}}</a>
                                                @endif
                                                @if(!empty($team[3]->name) and ($team[3]->email != $team_leader_row->email))
                                                    <a href="/competitions/wcodecode/remove/{{$team[2]->id}}">{{$team[3]->name}}</a>
                                                @endif
                                            </div>
                                        </div>
                                    @endif
                            ...
                            @if(!empty($user_row))
                                @if($user_row->wcodecode > 0 && empty($user_row->wcodecode_team))
                                ...
                                @if($user_row->wcodecode > 0 && !empty($user_row->wcodecode_team))
                                    <div class="" style="    border: solid 2px white;border-radius: 6px;padding: 2%;width: 95%">
                                        <p style="color: white">
                                            ...
                                            Team ID :WCO- {{$team_leader_row->wcodecode_team_id}}
