...
<head>
    ...
    <title>Cozmo Clench | Techfest, IIT Bombay</title>
    ...
    <style>
        ...
        @media (min-width:992px) {
            ...
            .mobile_hide{
                ...
                background-image: url(/2019/compi/images/cozmoclench.jpg) ;
                ...
                <div class="row ">
                    ...
                    <div class="col-md-8 animate-box" data-animate-effect="fadeInLeft" style="margin-right:-1%; padding-left:2.5% ;padding-right: -50%">
                        <div class="about-desc" style="margin-right: 0%; padding: 0px" >
                            ...
                            <h2 style="color: white; font-weight: bold">Cozmo Clench (Wildcard)</h2>
                            ...
                            <p style=" font-size: 15px; width:90%; font-family: Lato;color: white; text-align: justify ">
                                Your team must build a manually controlled bot which can do simple tasks of gripping blocks while overcoming the obstacles and completing the run by earning maximum points in minimum time. The bot can be wired or wireless. In case the participants use a wireless mechanism, they must use a dual frequency remote.
                            ...
                            @if(!empty($user_row->email))
                                ...
                                $team =  DB::table('tf_reg')->where(['wcozmo_team' => $user_row->wcozmo_team])->get();
                                $team_leader_row = DB::table('tf_reg')->where(['email' => $team[0]->wcozmo_team])->first();
                                if($user_row->wcozmo_team == $user_row->email ){
                                    $team_leader = $user_row->name;
                                }
                                ?>
                            @endif

                            <a href="/2019/compi/ps/Cozmo-Clench.pdf" target="_blank" class="btn btn-primary btn-learn">Problem Statement</a>
                            ...
                            @if(!empty($user_row->email))
                                ...
                                @endif
                                @if($user_row->wcozmo == 1 and empty($user_row->wcozmo_team))
                                    <div class="dropdown ">
                                        <button class=" btn btn-primary btn-learn"><b>Team</b></button>
                                        <div class="dropdown-content">
                                            <a href="/competitions/wcozmo/createteam">Create Team</a>
                                            <a href="/competitions/wcozmo/jointeam">Join Team</a>
                                        </div>
                                    </div>
                                @endif

                                @if(!empty($user_row->wcozmo_team))
                                    @if($user_row->wcozmo_team != $user_row->email )
                                        <a href="/competitions/wcozmo/leaveteam" class="btn btn-primary btn-learn" >Leave current Team</a>
                                    @endif
                                    @if($user_row->wcozmo_team == $user_row->email )
                                        <a href="/competitions/wcozmo/dissolveteam" class="btn btn-primary btn-learn" >Dissolve the team</a>
                                        <div class="dropdown ">
                                            ...
                                            <div class="dropdown-content">
                                                @if(!empty($team[0]->email) and ($team[0]->email != $team_leader_row->email))
                                                    <a href="/competitions/wcozmo/remove/{{$team[0]->id}}">{{$team[0]->name}}</a>
                                                @endif
                                                @if(!empty($team[1]->name) and ($team[1]->email != $team_leader_row->email))
                                                    <a href="/competitions/wcozmo/remove/{{$team[1]->id}}">{{$team[1]->name}}</a>
                                                @endif
                                                @if(!empty($team[2]->name) and ($team[2]->email != $team_leader_row->email))
                                                    <a href="/competitions/wcozmo/remove/{{$team[2]->id}}">{{$team[2]->name}}</a>
                                                @endif
                                                @if(!empty($team[3]->name) and ($team[3]->email != $team_leader_row->email))
                                                    <a href="/competitions/wcozmo/remove/{{$team[2]->id}}">{{$team[3]->name}}</a>
                                                @endif
                                            </div>
                                        </div>
                                    @endif
                            ...
                            @if(!empty($user_row))
                                @if($user_row->wcozmo > 0 and empty($user_row->wcozmo_team))
                                ...
                                @if($user_row->wcozmo > 0 and !empty($user_row->wcozmo_team))
                                    <div class="" style="    border: solid 2px white;border-radius: 6px;padding: 2%;width: 95%">
                                        <p style="color: white">
                                            ...
                                            Team ID :WCZ- {{$team_leader_row->wcozmo_team_id}}
...
<script>
    function myFunction2() {
        ...
        document.getElementById("text_change").innerHTML = "<br>Q. Why should I participate in Cozmo Clench? <br>" +
            ...
            "www.techfest.org > Competitions > Cozmo Clench > Explore More -> Register > Fill all your details > You will be registered and now you must either Create a team/Join a team<br>" +
...
<script>
    function myFunction4() {
        document.getElementById("text_change").innerHTML = "<br>Om Jaiswal <br>+91 7400400933 <br>omjaiswal@techfest.org<br><br> ";
        document.body.click("body");
